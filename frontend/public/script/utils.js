...
async function userRegister(ev){
    ev.preventDefault()
    const {isFormValidate,email,password} = validateUserRegisterForm()
    if(isFormValidate){
        ...
        await fetch('/api/emailverification',{
            ...
            'body' : JSON.stringify({
                'userType' : document.getElementById('student-checkbox').checked ? 'student' : 'staff' ,
                email,
                password,
                level,
                faculty
...
document.getElementById('admin-register-form').addEventListener('submit',userRegister)
